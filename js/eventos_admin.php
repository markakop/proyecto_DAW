...
if (isset($_POST['acc'])) {
	$acc=$_POST['acc'];
	if($acc==1){
		$result = Insertar("INSERT INTO products VALUES (".$_POST["id"].",'".$_POST["reference"]."',".$_POST["active"].",".$_POST["orden"].",".$_POST["price"].",'".(isset($_POST["code_manufacturer"])?$_POST["code_manufacturer"]:"null")."','".(isset($_POST["sku"])?$_POST["sku"]:"null")."',".$_POST['id_tax'].")");
	}
	if($acc==2){
	...
	if($acc==3){
		$result = Actualizar("UPDATE products SET price=".$_POST['price'].", reference='".$_POST['reference']."', orden=".$_POST['orden'].", active=".$_POST['active'].", sku='".$_POST['sku']."', code_manufacturer='".$_POST["code_manufacturer"]."', id_tax=".$_POST["id_tax"]." Where id=".$_POST['id']);
	}	
}
...
function modifica(idX,idP){
    ...
    frm.id_tax.value=eval("document.listado.id_tax"+idX).value;
...
<tr bgcolor="BDAC7C">
	...
	<!-- <td class="td-background" height="25"><SPAN CLASS="txtsubtitu">IVA<BR></SPAN><input class="margin-top-inpt" type="Text" size="3" name="iva" value=""></td> -->
	<?php $result_tax=Consultar("select * from tax  ORDER BY rate ASC"); 
	if(mysqli_num_rows($result_tax)>0){ ?>
		<td class="td-background" height="25"><SPAN CLASS="txtsubtitu">IVA<BR></SPAN>
			<select name="id_tax" id="rate-select">
				<option value="" disabled selected>--Escoge el IVA--</option>
				<?php while ($row = mysqli_fetch_array($result_tax)) { 
					echo '<option value="'.$row["id"].'">'.$row["rate"].'%</option>';
				} ?>
			</select>
		</td>
	<?php } ?>
...
<tr bgcolor="D9D9D9" class="fixed-row"><td height="25"><SPAN CLASS="txtsubtitu">Referencia</SPAN></td><td height="25"><SPAN CLASS="txtsubtitu">Precio</SPAN></td><td height="25"><SPAN CLASS="txtsubtitu">Código fabricante</SPAN></td><td height="25"><SPAN CLASS="txtsubtitu">SKU</SPAN></td><td height="25"><SPAN CLASS="txtsubtitu">Activo</SPAN></td><td><SPAN CLASS="txtsubtitu">Orden</SPAN></td><td><SPAN CLASS="txtsubtitu">IVA</SPAN></td><td></td><td></td><td></td></tr>
...
while($row = mysqli_fetch_array($result)) { 
	...
	$result_t=Consultar("select rate from tax  where id=".$row['id_tax']); 
	$row_t = mysqli_fetch_array($result_t)?>
	<td valign="top">
		<?php echo'<select name="id_tax'.$i.'" id="rate-select2">' ?>
			<option value="'<?php echo $row["id_tax"]; ?>'" disabled selected><?php echo $row_t['rate']; ?>%</option>
			<?php 
			$result_tax=Consultar("select * from tax  ORDER BY rate ASC"); 
			if(mysqli_num_rows($result_tax)>0){
				while ($row_tax = mysqli_fetch_array($result_tax)) { 
					echo '<option value="'.$row["id_tax"].'">'.$row_tax["rate"].'%</option>';
				} 
			}?>
		</select>
	</td>
...
<form method="post" name="acciones" action="admin/productos">
	...
	<input type="hidden" name="id_tax" value="">
